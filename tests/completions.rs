...
use organize_rs_testing::{files_differ, get_temp_file, TestResult};
...
pub static LAZY_RUNNER: Lazy<CmdRunner> = Lazy::new(|| {
    let mut runner = CmdRunner::new(env!("CARGO_BIN_EXE_ORGANIZE"));
...
fn fixtures_dir() -> PathBuf {
    ["tests", "fixtures", "completions"].iter().collect()
}
