...
- implement API like: <https://docs.rs/rust_search/2.1.0/rust_search/#structs> ?

- refactor filters to `Filter` trait
  - use `dyn Filter` (<https://crates.io/crates/enum_dispatch>)
- refactor dir walker to `DirWalk` trait
  - check if other trait for `location` is needed
  - use `dyn DirWalk` (<https://crates.io/crates/enum_dispatch>)
- refactor actions to `Action` trait
  - use `dyn Action` (<https://crates.io/crates/enum_dispatch>)
- refactor rules to `Rule` trait
  - use `dyn Rule` (<https://crates.io/crates/enum_dispatch>)
- refactor filesystem actions to `DryRunFileSystemInteraction` and
  `DestructableFileSystemInteraction` trait
